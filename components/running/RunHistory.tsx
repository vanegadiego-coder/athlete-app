  ...
  return (
    <div className="space-y-3">
      {kmBars.length > 0 && (
        <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-5">
          <div className="flex items-center justify-between mb-4">
            <p className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Km por semana</p>
            <p className="text-xs text-zinc-600">{kmBars[kmBars.length - 1]?.value || 0} km esta semana</p>
          </div>
      ...
      {bpmPoints.length >= 2 && (
        <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-5">
          <div className="flex items-center justify-between mb-4">
            <p className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Tendencia BPM</p>
            <p className="text-xs text-zinc-600">Promedio {avgBpm} bpm</p>
          </div>
          <LineChart points={bpmPoints} height={64} />
          <p className="text-xs text-zinc-700 mt-2">Baja = mejor condicion aerobica</p>
        </div>
      )}
      ...
      {runs.length > 0 && (
        <div className="bg-zinc-900 rounded-xl border border-zinc-800 overflow-hidden">
          <div className="px-5 py-3 border-b border-zinc-800">
            <p className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Carreras ({runs.length})</p>
          </div>
              ...
              return (
                <div key={run.id} className="px-5 py-3 flex items-center gap-4">
                  <div className="shrink-0 w-9 text-center">
                    <p className="text-xs text-zinc-700">{DAY_ES[d.getDay()]}</p>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      ...
                      <p className="text-xs text-zinc-500">{run.type}</p>
                    </div>
      ...
      {runs.length === 0 && (
        <div className="bg-zinc-900 rounded-xl border border-dashed border-zinc-800 p-10 text-center">
          <p className="text-sm text-zinc-600">Sin carreras registradas aun</p>
          <p className="text-xs text-zinc-700 mt-1">Toca + Carrera para empezar</p>
        </div>
      )}
    </div>
  );
